# AES-128 decryption from scratch (single 16-byte block) – based on Bo Zhu's minimal AES
# Challenge: decrypt given ciphertext with given key and print the flag
N_ROUNDS = 10

key        = b'\xc3,\\\xa6\xb5\x80^\x0c\xdb\x8d\xa5z*\xb6\xfe\\'
ciphertext = b'\xd1O\x14j\xa4+O\xb6\xa1\xc4\x08B)\x8f\x12\xdd'

# Forward S-box
s_box = [
]

# Inverse S-box
inv_s_box = [0]*256
for i,v in enumerate(s_box):
    inv_s_box[v] = i

# Round constants
Rcon = [0x00,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1B,0x36]

def sub_word(w):
    return ((s_box[(w>>24)&0xff]<<24) | (s_box[(w>>16)&0xff]<<16) | (s_box[(w>>8)&0xff]<<8) | s_box[w&0xff])

def rot_word(w): 
    return ((w<<8)&0xffffffff) | (w>>24)

def bytes_to_words(b):
    return [int.from_bytes(b[i:i+4], 'big') for i in range(0,16,4)]

def words_to_state(words):
    state = [[0]*4 for _ in range(4)]
    for c in range(4):
        w = words[c]
        state[0][c] = (w>>24)&0xff
        state[1][c] = (w>>16)&0xff
        state[2][c] = (w>>8)&0xff
        state[3][c] = w & 0xff
    return state

def state_to_bytes(state):
    out = bytearray(16)
    i=0
    for c in range(4):
        for r in range(4):
            out[i]=state[r][c]; i+=1
    return bytes(out)

def key_expansion(key_bytes):
    Nk=4; Nb=4; Nr=10
    w=[0]*(Nb*(Nr+1))
    key_words = bytes_to_words(key_bytes)
    for i in range(Nk):
        w[i]=key_words[i]
    for i in range(Nk, Nb*(Nr+1)):
        temp = w[i-1]
        if i%Nk==0:
            temp = sub_word(rot_word(temp)) ^ (Rcon[i//Nk]<<24)
        w[i]= w[i-Nk]^temp
    return w

def add_round_key(state, round_key_words):
    for c in range(4):
        k = round_key_words[c]
        state[0][c] ^= (k>>24)&0xff
        state[1][c] ^= (k>>16)&0xff
        state[2][c] ^= (k>>8)&0xff
        state[3][c] ^= k&0xff

def inv_shift_rows(state):
    state[1] = state[1][-1:] + state[1][:-1]
    state[2] = state[2][-2:] + state[2][:-2]
    state[3] = state[3][-3:] + state[3][:-3]

def inv_sub_bytes(state):
    for r in range(4):
        for c in range(4):
            state[r][c] = inv_s_box[state[r][c]]

def gmul(a,b):
    p=0
    for _ in range(8):
        if b & 1: p ^= a
        hi = a & 0x80
        a = (a<<1) & 0xff
        if hi: a ^= 0x1b
        b >>=1
    return p

def inv_mix_columns(state):
    for c in range(4):
        a0,a1,a2,a3 = (state[r][c] for r in range(4))
        state[0][c] = gmul(a0,14)^gmul(a1,11)^gmul(a2,13)^gmul(a3,9)
        state[1][c] = gmul(a0,9)^gmul(a1,14)^gmul(a2,11)^gmul(a3,13)
        state[2][c] = gmul(a0,13)^gmul(a1,9)^gmul(a2,14)^gmul(a3,11)
        state[3][c] = gmul(a0,11)^gmul(a1,13)^gmul(a2,9)^gmul(a3,14)

def decrypt(key, ciphertext):
    # Expand key
    w = key_expansion(key)

    # State from ciphertext
    state = words_to_state(bytes_to_words(ciphertext))

    # Initial AddRoundKey with last round key
    add_round_key(state, w[40:44])

    # Rounds 9..1
    for round in range(9, 0, -1):
        inv_shift_rows(state)
        inv_sub_bytes(state)
        add_round_key(state, w[round*4:round*4+4])
        inv_mix_columns(state)

    # Final round (no InvMixColumns)
    inv_shift_rows(state)
    inv_sub_bytes(state)
    add_round_key(state, w[0:4])

    # Back to bytes
    return state_to_bytes(state)

if __name__ == "__main__":
    plaintext = decrypt(key, ciphertext)
    print(plaintext.decode())